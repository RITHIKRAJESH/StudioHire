...
export default function Userregister() {
 ...
 useEffect(()=>{
  ...
  const findErrors = () => {
    ...
    const { fullname, address,email, phone, password, expertise, cpass,} = record;
    const newErrors = {};
    ...
    if (!expertise || expertise === '') {
        newErrors.job = 'Cannot be blank..!';
    } else if (expertise.length > 30) {
        newErrors.job = 'name is too long!';
    }
  ...
  const handlerSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
        ...
        //alert("else submit")
      const url = "http://localhost:8000/userreg";
  ...
  const handleGoBack = () => {
    ...
    return (
        <div>
            ...
            <Form onSubmit={handlerSubmit}  encType='multipart/form-data' className="bg-light" >
        
                <Container className="">
                <Row>
                        <Col>
                            <h1 className='hd'><b>APPLY TO WORK WITH US...!</b></h1>
              ...
              <Form.Group>
                ...
                />
                        ...
                        <Form.Group>
                            ...
                            <select name="expertise"onChange={(e) => setValue(e.target.name, e.target.value)}>
                                <option>Select Expertise</option>
                                {
                                    category
                                    .map((ls)=>{
                                        return(
                                    <option value={ls.category}>{ls.category}</option>
                                        )
                                    })
                                }
                            </select>
                            {/* <Form.Control type="text" name="expertise" placeholder="Enter your expertise" onChange={(e) => setValue(e.target.name, e.target.value)} isInvalid={!!errors.job} /> */}
                            <Form.Control.Feedback type='invalid'>
                                {errors.job}
                            </Form.Control.Feedback>
                        </Form.Group> 
